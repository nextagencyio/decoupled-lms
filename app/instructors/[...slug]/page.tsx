...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Instructor Not Found | Lms' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).coursesTaught && (
                    <div>
                      <dt className="text-sm text-gray-500">Courses Taught</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).coursesTaught}</dd>
                    </div>
                  )}
                  {(item as any).totalStudents && (
                    <div>
                      <dt className="text-sm text-gray-500">Total Students</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).totalStudents}</dd>
                    </div>
                  )}
                  {(item as any).instructorRating && (
                    <div>
                      <dt className="text-sm text-gray-500">Instructor Rating</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).instructorRating}</dd>
                    </div>
                  )}
