...
import { GET_COURSE_BY_PATH } from '@/lib/queries'
import { DrupalCourse } from '@/lib/types'
...
async function getCourse(path: string): Promise<DrupalCourse | null> {
  try {
    ...
    const { data } = await apolloClient.query<CourseByPathData>({
      query: GET_COURSE_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/courses/${slug.join('/')}`
  const item = await getCourse(path)

  if (!item) {
    return { title: 'Course Not Found | Lms' }
  }

  return {
    title: `${item.title} | Lms`,
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).instructorName && (
                    <div>
                      <dt className="text-sm text-gray-500">Instructor Name</dt>
                  ...
                  {(item as any).lessonsCount && (
                    <div>
                      <dt className="text-sm text-gray-500">Number of Lessons</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).lessonsCount}</dd>
                    </div>
                  )}
                  {(item as any).price && (
                    <div>
                  ...
                  {(item as any).enrollmentCount && (
                    <div>
                      <dt className="text-sm text-gray-500">Enrollment Count</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).enrollmentCount}</dd>
                    </div>
                  )}
                  {(item as any).rating && (
                    <div>
